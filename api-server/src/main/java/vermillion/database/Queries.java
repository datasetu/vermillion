...
public class Queries {
    ...
    public Queries() {
        baseQuery = new JsonObject().put("query", new JsonObject().put("bool", new JsonObject().put("filter", "")));

        filterQuery = new JsonArray();

        timeQuery = new JsonObject()
                .put(
                        "range",
                        new JsonObject()
                                .put(
                                        "timestamp",
                                        new JsonObject().put("gte", "").put("lte", "")));

        geoQuery = new JsonObject()
                .put("geo_distance", new JsonObject().put("distance", "0km").put("coordinates", new JsonArray()));
    ...
    public JsonObject getLatestQuery(JsonObject queryObject) {
        return queryObject
                ...
                .put(
                        "sort",
                        new JsonArray().add(new JsonObject().put("timestamp", new JsonObject().put("order", "desc"))));
    }
