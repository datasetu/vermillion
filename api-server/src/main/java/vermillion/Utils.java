...
public class Utils {
  ...
  static {
    if ("true".equals(System.getenv("SINGLE_NODE"))) single_node = true;
  ...
  public static String getBrokerUrl(String username) {
    ...
    if (single_node) {
      ...
      /**
       * Using a simple hash function m % n to distribute load between rabbitmq nodes. There are 26
       * characters and "n" broker nodes in the swarm. If we assign a=1, b=2, c=3 and so on, this
       * operation becomes straightforward. Thus to achieve this, we take the ASCII representation
       * of the first letter of the username and subtract it with 96 (since lowercase ASCII numbers
       * start from 97 and we want the index to start from 1.) Once we have the index, perform the
       * mod operation to get the bucket index. Since this operation is consistent across all broker
       * operations, there is a strong guarantee that we exactly resolve to the appropriate broker
       * node under all circumstances.
       *
       * <p>For e.g. if the swarm cluster has 6 brokers in it and a users named "alpha" and "quebec"
       * wish to operate on the broker then, for the user "alpha":
       *
       * <p>(int)username[0] = 97 (ASCII representation of a) 97 - 96 = 1 1 % 6 = 1
       *
       * <p>Thus, the user "alpha" is connected with "rabbit1"
       *
       * <p>Now for the user "quebec",
       *
       * <p>(int)username[0] = 113 113 - 96 = 17 17 % 6 = 5
       *
       * <p>Likewise, the user "quebec" is connected with rabbit5
       *
       * <p>If the number of broker nodes is a factor of the integer representation of the
       * character, then the value of "broker_nodes" is taken as the bucket number. For e.g. for the
       * user "romeo", the bucket number resolves to 0, thus we add 6 to it and connect them to
       * rabbit6
       */
