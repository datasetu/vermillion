...
s_id = r['scroll_id']
requested_id1=[]
acl_set_policy4 = "consumer@iisc.ac.in can access example.com/test-category/" + resource_ids[
    13] + " for 1 month"
data = {"policy": acl_set_policy4}
# print(data)
response = requests.post(
    'https://localhost:8443/auth/v1/acl/set',
    headers=headers,
...
requested_id1.append({
    "id": id_prefix + resource_ids[13],
    "scopes": ["read", "write"]
})

data = {"request": requested_id1}
# print(data)
response = requests.post(
    'https://localhost:8443/auth/v1/token',
    headers=headers,
